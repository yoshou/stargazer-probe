...
{
    /// <summary>
    /// Sends camera frames + latest IMU via gRPC (Client Streaming).
    /// 
    ...
    /// - Only discards old messages when queue exceeds limit (to preserve memory limit)
    /// </summary>
    public class GrpcDataStreamer : MonoBehaviour
    {
        ...
        private readonly ConcurrentQueue<Stargazer.CameraImageMessage> cameraQueue = new ConcurrentQueue<Stargazer.CameraImageMessage>();
        private readonly ConcurrentQueue<Stargazer.InertialMessage> inertialQueue = new ConcurrentQueue<Stargazer.InertialMessage>();
        ...
        private void StartSendLoop()
        {
            ...
            while (cameraQueue.TryDequeue(out _)) { }
            while (inertialQueue.TryDequeue(out _)) { }
        ...
        private async Task SendLoopAsync(CancellationToken cancellationToken)
        {
            while (!cancellationToken.IsCancellationRequested)
            {
                ...
                if (!cameraQueue.TryDequeue(out var cameraMsg))
                {
                    if (!inertialQueue.TryDequeue(out var inertialMsg))
                        continue;
                    ...
                    try
                    {
                        await grpcClient.SendInertialAsync(inertialMsg).ConfigureAwait(false);
                        Interlocked.Increment(ref framesSent);
                    }
                    catch (Exception ex)
                    {
                        Interlocked.Increment(ref sendErrors);
                        Debug.LogError($"[GrpcDataStreamer] SendInertial failed: {ex.Message}");
                    }
                    continue;
                }
                ...
                try
                {
                    await grpcClient.SendCameraImageAsync(cameraMsg).ConfigureAwait(false);
                    Interlocked.Increment(ref framesSent);
                }
                catch (Exception ex)
                {
                    Interlocked.Increment(ref sendErrors);
                    Debug.LogError($"[GrpcDataStreamer] SendCameraImage failed: {ex.Message}");
                }
            }
        }
        ...
        private void OnFrameCaptured(CameraFrameDataWithIMU frameWithIMU)
        {
            ...
            string deviceId = string.IsNullOrWhiteSpace(deviceIdOverride) ? SystemInfo.deviceUniqueIdentifier : deviceIdOverride;

            // Create camera message
            var cameraMsg = new Stargazer.CameraImageMessage
            {
                Timestamp = (long)frameWithIMU.FrameData.Timestamp,
                Name = deviceId
            };
            cameraMsg.Values.Add(ProtoConverters.ToProtoCameraImage(frameWithIMU.FrameData));

            cameraQueue.Enqueue(cameraMsg);
            int q = Interlocked.Increment(ref queuedPackets);
            sendSignal.Release();

            // Create inertial message if we have IMU samples
            if (frameWithIMU.IMUSamples != null && frameWithIMU.IMUSamples.Length > 0)
            {
                var inertialMsg = new Stargazer.InertialMessage
                {
                    Timestamp = (long)frameWithIMU.FrameData.Timestamp,
                    Name = deviceId
                };
                ...
                {
                    inertialMsg.Values.Add(ProtoConverters.ToProtoInertial(imuSample));
                }

                inertialQueue.Enqueue(inertialMsg);
                q = Interlocked.Increment(ref queuedPackets);
                sendSignal.Release();
            }

            int maxBuffer = config != null && config.Advanced != null ? Mathf.Max(1, config.Advanced.MaxBufferSize) : 100;

            // If queue grows beyond cap, drop oldest messages
            while (q > maxBuffer)
            {
                ...
                else if (inertialQueue.TryDequeue(out _))
                {
                    q = Interlocked.Decrement(ref queuedPackets);
                }
                else
